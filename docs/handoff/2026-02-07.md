...
### Iteration 9 (Fix: better-sqlite3 ABI Mismatch in Dev)

- In dev, the engine now starts with system `node` (matches the `better-sqlite3` binary built by `npm install`) instead of Electron’s Node runtime (`apps/ide/main.js`).

### Iteration 10 (Fix: SQLite Schema Syntax Error)

- Fixed a trailing-comma schema bug in `apps/backend/src/database.ts` that caused `SqliteError: near ")": syntax error`.

### Iteration 11 (Threads + Runs Persistence)

- Migrated SQLite table naming from `sessions*` → `threads*` with automatic in-place migrations on startup (`apps/backend/src/database.ts`).
- Added persisted `runs` + `run_events` tables and wired generation replay to read from the latest persisted run (append-only, replayable) (`apps/backend/src/ipcServer.ts`).
