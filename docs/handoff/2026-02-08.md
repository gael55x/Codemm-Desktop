...
### Iteration 36 (Generation Robustness: Eliminate Non-Healing Contract Loops)

- Java `brittle whitespace` self-heal now trims boundary whitespace for **all** string literals in `test_suite`, not just `assertEquals(...)` expected strings (prevents repeated failures when whitespace appears in messages/other assertions) (`apps/backend/src/generation/perSlotGenerator.ts`).
- Removed the contract-time requirement that `test_suite` must explicitly reference the target class name; Docker validation is the real guardrail and the prior string check caused repeated identical failures across attempts (`apps/backend/src/generation/perSlotGenerator.ts`).

### Iteration 37 (Generation Robustness: Reference Solution Repair Mode)

- When Docker validation fails for Java legacy problems, retries now repair **only** `reference_solution` (instead of regenerating the entire problem JSON), using the JUnit output + a summarized failure list as guidance (reduces “same wrong solution” loops and makes each attempt meaningfully corrective) (`apps/backend/src/generation/perSlotGenerator.ts`).
- Repair attempts use a slightly higher temperature to explore fixes when stuck (`apps/backend/src/generation/perSlotGenerator.ts`).
